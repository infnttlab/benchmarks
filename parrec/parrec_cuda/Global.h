...
//	#define ZERO_F 0. ora si trova in global.c come MIN_FLOAT
	...
	#define U8 5   	// non pi� gestito come U16 
	#define FLT 3 	// corrispondente a LLNL 3
	#define RGB_ 6  	// attualmente non in uso ma serve per immagini a colori
#define DATATYPE_STRING "1=U16, 5=U8, 3=FLOAT, 6=RGB"   // cos� se cambio qui qualcosa cambia automaticamente ovunque
	//-----------------------------------------------------------------------------
	#define SDT 1
	#define TIF 2
	#define JPG 3
...
#define FILETYPE_STRING "1=SDT 2=TIF 3=JPG 4=BMP" // idem sopra
	...
	//-----------------------------------------------------------------------------
	// define per scoprire se il file dei link c'� non c'� � completo o no
	//-----------------------------------------------------------------------------
	#define	NOT_KNOWN							0
	...
	//-----------------------------------------------------------------------------
	// define di int TomographicType  // indica lo step tomografico, cio� il tipo 
	// 						di immagini pu� essere projection atenrad sinos recobj
	//-----------------------------------------------------------------------------
	#define PROJECTION 1   
	#define ATENRAD 2
	#define SINOS 3
	...
	//-----------------------------------------------------------------------------
	// � un tipo di DataSet.numberofimages
	...
	//-----------------------------------------------------------------------------
	#define SPIKES					14	// contrassegno che quel punto � spikes
	#define NO_SPIKES				15	// contrassegno che quel punto NON � spikes
	#define SPIKES_RIPESCATO		16	// spikes in pi�
	#define SPIKES_ORIZZONTALE		17	// spikes in pi� in orizzontale
	...
	//-----------------------------------------------------------------------------
	// DataSet.current = -9 se non � una sequenza (pu� anche essere singola immagine ma con u numero e una radice)
	#define NO_SEQ -9
	#define DARK -1
	#define IZERO -2
	//-----------------------------------------------------------------------------
	...
	#define MKRECONSTRUCT_NEWMETHOD	22	 // ricostruzione parallela parallelizzando i pixel della slice NON PI� ATTIVO!!!!
	...
	struct data_tomo		
	{
		...
		int Single_Image_inSequence;	// se � TRUE allora anche se � una singola immagine ha un numero altrimenti no
		...
		int		TomographicType;  // indica il tipo tomografico, cio� il tipo di immagini. Pu� essere projection atenrad sinos recobj
		int		StrangeName;	  // indica che il nome � diverso (non projection, atenrad,etc..) ma che � classificato come step tomografico
		...
		char	Name[SHORT2_STRING];	// nome del file : se � una sequenza senza "_"
		...
		int		FFT_do_invFFT;   // se � TRUE (come di default) dopo fa anche l'antitrasformata se no no
		int		FFT_do_FeldkampWeight;  // se � TRUE applica anche il peso per Feldkamp (necessario per ricostruire)
	// ------------------------------------------------------------------------------------------------------
	...
	// ------------------------------------------------------------------------------------------------------
		...
		int		nslices;	// SCT: � il numero di righe del rivelatore ovvero di slices, cio� l'altezza del rivelatore o di projection o di atenrad o il numero di sinogrammi
		int		nangles;	// SCT: � il numero di angoli cio� il numero di step angolari ripresi 
		int		nrays;		// SCT: � il numero di colonne cio� la largehzza del rivelatore deve essere SEMPRE uguale a dataset.width
		float		sdd;		// SCT: � la distanza frala sorgente e rivelatore   mm
		float		odd;		// SCT: � la distanza frala oggetto e rivelatore (mm)
		float		sod;		// SCT: � la distanza frala sorgente e oggetto (mm)
		...
		int		rec_slice_circle_present;	// se � TRUE sono presenti in memoria x_start,x_end per la ricostruzione dentro al cerchio della slice
		int		rec_slice_circle_use;		// se � TRUE vanno usati x_start,x_end per la ricostruzione dentro al cerchio della slice
		char	recobj_name[SHORT2_STRING];	// nome del file della recobj: se � una sequenza senza "_"
		int		rec_try_pxcenter;      // se � TRUE allora il numero della sequenza corriponde a un centro diverso
		float	pxcenter_start;
		float	pxcenter_end;
		float	pxcenter_step;		
		int		localCT_status;		// se � TRUE si vuole ricostruire in localtomography
...
//-----------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------
	extern int			n_pxcenter;   // mi serve per dire che indice devo salvare quando trycenter � attivo
