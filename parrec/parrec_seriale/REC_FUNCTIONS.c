...
float  *row_re=NULL,*row_im=NULL;			// le due parti reale e immaginaria della riga
...
int BackProjectFilteredDataOnSlice(int FFT_lenght,int FFT_start,float xs,float ys,float	xd0,float yd0,float	xd1,float yd1) {
	...
	float	U,U_quadro;						// pesatura per EQUISPAZIATI: rapporto fra sod e distanza della proiezione del punto della slice 
...
//	char	txt[MEDIUM_STRING];
	...
	for(h=0; h<(int)nangles; h++) {
		...
		for(k=FFT_start; k<FFT_start+width; k++) {// qui ci va width e non nrays altrimenti in halfscan non fa bene il filtro
			row_re[k]=DataSet.Image_FL[riga+k-FFT_start];
		}
		// pesatura per equispaziati SOLO FAN BEAM ---W1---
		for(k=FFT_start; k<FFT_start+width; k++) // qui ci va width e non nrays altrimenti in halfscan non fa bene il filtro
			row_re[k] *= detector_weighing[k-FFT_start];
		
		//se FFT_lenght (dimensione di row_re e row_im) è minore uguale a 2^18 si fa la fft del NR, altrimenti la FFTW
		if(FFT_lenght<=pow(2,18)){
			...
			input = (fftw_complex*)fftw_malloc(sizeof(fftw_complex)*FFT_lenght);
			
			int i;
			//costruisco input(come lo si fa con CUDA)
			for(i=0;i<FFT_lenght;i++){
				input[i][REAL] = row_re[i];
				input[i][IMAG] = row_im[i];
			}
			
			my_plan = fftw_plan_dft_1d(FFT_lenght, input, input, FFTW_BACKWARD, FFTW_ESTIMATE);
			
			fftw_execute(my_plan);
			
			// convoluzione con il filtro 
			for(k=0; k<FFT_lenght; k++){
				input[k][REAL] = input[k][REAL]*filter[k];
				input[k][IMAG] = input[k][IMAG]*filter[k];
			}
			
			my_plan = fftw_plan_dft_1d(FFT_lenght, input, input, FFTW_FORWARD, FFTW_ESTIMATE);
			
			fftw_execute(my_plan);
			
			for(i=0;i<FFT_lenght;i++){
				row_re[i] = input[i][REAL];
				row_im[i] = input[i][IMAG];
			}
