...
int main(int argc, char* argv[]) {
	...
	DataSet.current_image=400;
	DataSet.Seq_start = 400;
	DataSet.Seq_end = 400;
	...
	// si vede se c'è il file sct e nel caso si carica	
	r = Open_SCT_File();
	if( r < 0 )		return;

	// unica funzione per ricostruire, usa in ogni caso x_start e x_end ma se è stato calcolato il cerchio della slice 
...
int Open_SCT_File(void) {
	...
	// si vuole cercare automaticamente se c'è già
	// un file SCT che vada bene cioè che abbia il nome della sequenza 
...
int Open_SDT_File(int n) { 
	...
	if (DataSet.Data_Type == U16 ) {
		...
		// rossella rossella al momento non gestiamo dark nè I0 a 8 bit
...
int OpenSPRFile(char *PathName) {
	...
	data_dim = atoi ( &buf[0] ); // atoi ( &buf[endline_position-1] );			   tanto è sempre e solo un numero, il primo!!!
...
// se in path c'è tomostep allora si salva automaticamente il file in memoria e SOVRASCRIVE
...
// ritrna -2 se il tipo di file non è SDT salvabile
//-----------------------------------------------------------------------------
...
int Save_SPR_File(char *path) {
	...
	fprintf(f,"0.000000\n");
	fprintf(f,"0.000000\n");
//	if( strcmp(path,"sinos") == 0 ) // se stiamo scrivendo i sinogrammi height è il numero di atenrad oppure il range angolare rossella rossella
...
extern void Initialize_Data_DataSet(int dataset) {
	...
	switch(dataset) {
		...
		case(DATASET1) : {
			...
			DataSet.new_dataset 		= TRUE; // è un nuovo data set e quindi si rialloca
			...
			sprintf(DataSet.recobj_name,"recobj_ok");	// nome del file della recobj: se è una sequenza senza "_"
			DataSet.rec_try_pxcenter = FALSE;
			DataSet.pxcenter_start   = 0.;
			DataSet.pxcenter_end     = 0.;
			DataSet.pxcenter_step    = 0.;		
			DataSet.localCT_status=FALSE;		// se è TRUE si vuole ricostruire in localtomography
		...
		case(DATASET2) : {
			...
			DataSet2.new_dataset 		= TRUE; // è un nuovo data set e quindi si rialloca
			...
			DataSet2.rec_slice_circle_present	= FALSE;	// se è TRUE sono presenti in memoria x_start,x_end per la ricostruzione dentro al cerchio della slice
			DataSet2.rec_slice_circle_use		= TRUE;	// se è TRUE vanno usati x_start,x_end per la ricostruzione dentro al cerchio della slice
			sprintf(DataSet2.recobj_name,"recobj_ok");	// nome del file della recobj: se è una sequenza senza "_"
			DataSet2.rec_try_pxcenter = FALSE;
			DataSet2.pxcenter_start   = 0.;
			DataSet2.pxcenter_end     = 0.;
			DataSet2.pxcenter_step    = 0.;		
			DataSet2.localCT_status=FALSE;		// se è TRUE si vuole ricostruire in localtomography
...
//	double	n; // n è definito in modo che sia zero in pxcenter
	...
	// la slice è quadrata di lato width
	width = DataSet.Width;
	new_width = width;
	new_height = width;
	...
	free(row_re);				row_re = NULL;				row_re = (double *) malloc ( sizeof(double)*FFT_lenght+1);
	free(row_im);				row_im = NULL;				row_im = (double *) malloc ( sizeof(double)*FFT_lenght+1);
	free(filter);				filter = NULL;				filter = (double *) malloc ( sizeof(double)*FFT_lenght+1);
	free(detector_weighing);	detector_weighing = NULL;	detector_weighing = (double *) malloc ( sizeof(double)*width+1);
	// --------------------------------------------------
	...
	// coordinate del centro dell'oggetto: con il nuovo metodo mi serve solo per la visualizzazione perchè poi non lo userò in quanto nulle

	// dati per FFT: abbiamo già calcolato la lunghezza necessaria per la FFT ora calcoliamo il punto dove inserire
	// la proiezione che si troverà al centro dell'array nello spazio delle frequenze
	...
	W2=1.; // peso ---W2--- dovrebbe essere: W2=0.5/(double)pxsize	//attenzione che questo filtro è troppo pesante! va meglio con W2=1.
	CalculateFourierFilter(FFT_lenght, FFT_start, W2);

	// calcolo del peso per equispaziati U --- W3 --- N.B. il calcolo di U è dentro a BackProjectFilteredDataOnSlice
	//CalcolateWeighing_EquispacedRays(sod,rxsize);

	// calcolo del peso nel caso di halfscan andrebbe messo qui e moltiplicato per W1 così non devi nè memorizzarlo nè ricalcolarlo ogni volta
...
//******************************************************************************
	...
	// se si stanno elaborando più file (o anche uno solo ma non c'è preview) bisogna caricare il file da ricostruire
	Open_SDT_File(image);
