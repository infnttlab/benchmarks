...
void add_vector_in_matrix(LD *matrix, LD *M_in, ULI col, LD time, int membid, struct globale *G) {
  matrix[(col+2)*G->uliRow_counter[membid]] = time;
  matrix[((col+2)*G->uliRow_counter[membid]) + 1] = G->ldFS[membid];  
  memcpy(&(matrix[( (col+2)*G->uliRow_counter[membid] )+2]), M_in, ( sizeof(LD)* col ));
...
void write_matrix(FILE * fPtr, LD *matrix, ULI cols, int membid, struct globale *G) { 
  ...
  sprintf(file,"output/multi_%d",membid);
  if ((G->m_fPtr[membid] = fopen (file,"a")) == NULL) {
	printf ("\nCannot write in the current directory - Output file\nAbort\n");
	exit(0);
  }
	
  fwrite(matrix, sizeof(LD)*(cols+2),G->uliRow_counter[membid],G->m_fPtr[membid]);
  G->uliRow_counter[membid] = 0;
  
  fclose(G->m_fPtr[membid]); 
}



void write_result(struct globale *G, LD time, int membstart, int membstop) {
  FILE* fPtr;
  ULI col;
  int i, j;
  
  if ((fPtr = fopen ("output/multi_final.txt","w")) == NULL) {
	printf ("\nCannot write in the current directory - Output file\nAbort\n");
	exit(0);
  }
  ...
  for(i=membstart; i<=membstop; i++) {
	col = G->uliColumns[i];
  	fprintf(fPtr,"mem%4d: time %10LG FS %10LG\t",i,time,G->ldFS[i]);
	for (j=0;j<col; j++) fprintf(fPtr,"%4LG\t",G->M[i][j]);
	fprintf(fPtr,"\n");
  }
...
int load_files(int idf, int base, struct globale *G) {
  ...
  if(G->uliColumns[id] != G->uliIndexCol) printf("DIVERSI %lu %lu\n", G->uliColumns[id],G->uliIndexCol);
