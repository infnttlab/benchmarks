...
void initialize(LD uliStep_max, gsl_rng *r, struct globale *G) {	
  ...
  for(k=nMstart;k<=nMstop;k++) {  
	...
	*/
	...
	//Check 
	if(!kk) printf("We have %d membranes, %lu reactions, %lu species\n", nMnum, G->uliRows[kk], G->uliColumns[kk]);
	else if (G->uliRows[kk] != G->uliRows[0]) printf("****** WARNING! ****** %5d: %5lu reactions, %5lu species\n",kk, G->uliRows[kk], G->uliColumns[kk]);
	...
	G->ldM_matrix[kk] = (LD *) malloc(sizeof(LD) * (G->uliColumns[kk]+2)*(G->buffer_lines)); //era +1, ma ora anche ldfs
	buffer_size += sizeof(LD)*(G->uliColumns[kk]+2)*(G->buffer_lines);
	...
	t1 = (LD) G->uliIndexCol+2; //era +1
	fwrite(&t1,sizeof(long double),1,G->m_fPtr[kk]);

	fclose(G->m_fPtr[kk]); //just to create it
  ...
  while (t < uliStep_max) {										//92.4%%
	...
	//aggiungere meccanismo di uscita in caso di loop (non riesco ad avere un FS positivo, o sul tau == 0 o sul numeor id giri
	do {
		...
		if(cn == 1) {
			///
			///printf("step3 neg... from tau %LG ",G->tau[0]); fflush(NULL);	
	...
	for(k=nMstart;k<=nMstop;k++) {								//53.8%%
		...
		if(t3 < t1) t3 = t1;
  		...
  		//fprintf(G->fs_fPtr[kk], "%LG\n", G->ldFS[kk]); scritto in dpp_step4
	}
	t = t3;
  }
  ...
  for(k=nMstart;k<=nMstop;k++) {
	...
	//Write always the last status. This means that the final status may appear two times in the result file
	add_vector_in_matrix(G->ldM_matrix[kk],G->M[kk],G->uliColumns[kk],t,kk,G);
  ...
  write_result(G, t, nMstart, nMstop);
}
