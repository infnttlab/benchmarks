...
void feeding (int membid, struct globale *G) {
  ...
  aux = membid*G->uliColumns[membid];
  for(i = 0; i < G->uliColumns[membid]; i++)
  	  if (G->ldM_feed[aux+i] > 0) G->M[membid][i] = G->ldM_feed[aux+i];
}
...
LD getFS(int membid, struct globale *G) {
  ...
  for(i = 0; i<G->uliColumns[membid]; i++) G->ldFS[membid] -= (G->M[membid][i] * G->ldMolSize[aux+i]);
...
void rule_prob(int membid, LD t, int uliStep, struct globale *G) {
  ...
  //memcpy(G->ldR_prob[G->side_index2[membid]], G->ldC_vector[G->side_index2[membid]], G->uliRows[membid] * sizeof(LD));
  
  aux = G->side_index2[membid];
  for(c=0;c<G->uliRows[membid]; c++) G->ldR_prob[aux+c] = G->ldC_vector[aux+c];
  ...
  for(c=0; c<G->uliRows[membid]; c++) {
  	...
  	if (G->iTgt_vector[membid][c] == -1) {
  	        G->ldR_prob[G->side_index2[membid]+c] /= G->ldFS[membid];
  	        G->ldR_prob[G->side_index2[membid]+c] /= G->ldMembSize[membid];
  	}
	...
	for(d=0; d<G->uliColumns[membid]; d++) {
			...
			case 0: 
				break;
  	        	case 1:
  	        		G->ldR_prob[G->side_index2[membid]+c] *= G->M[membid][d];
  	        		break;
  	        	case 2:
  	        		G->ldR_prob[G->side_index2[membid]+c] *= (G->M[membid][d] * (G->M[membid][d] - 1)) * 0.5;
  	        		break;
  	        	default:
  	        		//for(e=1; e<=G->unLeft_side[membid][c][d];e++)
				for(e=1; e<=G->unLeft_side[G->side_index[membid]+(c*G->uliColumns[membid])+d];e++)
  	        			G->ldR_prob[G->side_index2[membid]+c] *= (G->M[membid][d] - e + 1.0) / (1.0*e);
  	        		break;
  	        }	
	}
  }
...
void dpp_tossing(LD a_0, gsl_rng *r, int membid, struct globale *G) {
  ...
  alpha = G->ldR_prob[G->side_index2[membid]+rule];
...
int dpp_step1(gsl_rng *r, int membid, LD* a0c, LD* t2, LD t, int uliStep, struct globale *G) {
  ...
  for(j=0;j<G->uliRows[membid];j++) G->ldR_prob[G->side_index2[membid]+j]=0.0; 
  ...
  for(j=0;j<G->uliRows[membid];j++) a_0 += G->ldR_prob[G->side_index2[membid]+j];
...
void dpp_step3(gsl_rng *r, int membid, struct globale *G, int nMembranes) {	
  ...
  if(G->iFlagStep[membid] == 2 || G->iFlagStep[membid] == 3)
	for(j = 0; j < G->uliRows[membid]; j++)
		if (G->uliCritical[membid][j] == 0) G->uliK_rule[membid][j] = gsl_ran_poisson(r, G->tau[membid] * G->ldR_prob[G->side_index2[membid]+j]);
...
*/
  ...
  for(k = 0; k < G->uliRows[membid]; k++) {
  	if (G->uliCritical[membid][k] == 0) {
		for(j = 0; j < G->uliColumns[membid]; j++) {
			ivaux = G->iVar[G->side_index[membid]+(k*G->uliColumns[membid])+j];
			ivaux2 = G->ldR_prob[G->side_index2[membid]+k];
  			mu[j]    +=     (LD)(ivaux)    * ivaux2;
  			sigma[j] += pow((LD)(ivaux),2) * ivaux2;
		}
  	}
  }
  ...
  if (tau1 < 10.0 * (1.0/ a_0)) {
  	...
  	/*Get the smallest tau among the processes*/
       ...
       for(k = 0; k < G->uliRows[membid]; k++) if (G->uliCritical[membid][k] == 1) a_0_c += G->ldR_prob[G->side_index2[membid]+k];
  ...
  if(G->iFlagStep[membid] == 2) {
  	  for(i = 0; i < G->uliRows[membid]; i++)
  		  if (G->uliCritical[membid][i] == 0)
  			  G->uliK_rule[membid][i] = gsl_ran_poisson(r, G->tau[membid] * G->ldR_prob[G->side_index2[membid]+i]);
  }
  else if(G->iFlagStep[membid] == 3){ //else 2
  	  for(i = 0; i < G->uliRows[membid]; i++) {
  		  if (G->uliCritical[membid][i] == 0) {
  			  G->uliK_rule[membid][i] = gsl_ran_poisson(r, G->tau[membid] * G->ldR_prob[G->side_index2[membid]+i]);
  			  G->ldR_prob_c[G->side_index2[membid]+i] = 0.0;
  		  }
  		  else G->ldR_prob_c[G->side_index2[membid]+i] = G->ldR_prob[G->side_index2[membid]+i];
  	  }
  	  i=0;
  	  ...
  	  if(tau2[membid] == G->tau[membid]) {
  		  ...
  		  alpha = G->ldR_prob_c[G->side_index2[membid]+i];
  		  while(rnd_t > alpha) {
  			  i++;
  			  alpha += G->ldR_prob_c[G->side_index2[membid]+i];
  		  }
